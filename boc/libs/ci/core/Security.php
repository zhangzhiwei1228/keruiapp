...
class CI_Security {
	...
	public function csrf_verify()
	{
		...
		# 过滤列表
		$filters_ctrls = config_item('csrf_filter');

		// #CODE: 对 ueditor 过滤 csrf
		if ($filters_ctrls and in_array($uri[0],  $filters_ctrls) ) {
		} else {
	...
	public function xss_clean($str, $is_image = FALSE)
	{
		...
		if (config_item('security_naughty_html')) {
			$naughty = config_item('security_naughty_html');
		}
	...
	protected function _remove_evil_attributes($str, $is_image)
	{
		...
		// #CODE: 对于含有 img 标签而允许 style的通过 去除了'style' 属性的过滤，有一定风险
		// if ($have_img) {
