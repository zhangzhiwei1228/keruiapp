<div class="btn-group">
    <a href="<?php echo site_urlc('msg/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
<table class="table table-striped table-hover select-list">
    ...
    <tbody class="sort-list">
        ...
        <tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
            ...
            <td> <?php echo msubstr(strip_tags($v['content']),0,35) ?></td>
...
require(['adminer/js/ui'],function(ui){
  var msg = {
    url_del: "<?php echo site_urlc('msg/delete'); ?>"
    ,url_audit: "<?php echo site_urlc('msg/audit'); ?>"
    ,url_flag: "<?php echo site_urlc('msg/flag'); ?>"
    ,url_sortid: "<?php echo site_urlc('msg/sortid'); ?>"
  };

  ui.fancybox_img();
  ui.btn_delete(msg.url_del);     // 删除
  ui.btn_audit(msg.url_audit);    // 审核
  ui.btn_flag(msg.url_flag);      // 推荐
  ui.sortable(msg.url_sortid);    // 排序
});
</script>
