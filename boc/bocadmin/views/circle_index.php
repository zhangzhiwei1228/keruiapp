<?php if (false): ?>
<div class="btn-group">
	<a href="<?php echo site_urlg('circle/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
<?php endif; ?>
...
require(['adminer/js/ui', 'tools'],function(ui, tools){
  tools.selectLink.init({
	...
	var circle = {
		url_del: "<?php echo site_urlg('circle/delete'); ?>"
		,url_audit: "<?php echo site_urlg('circle/audit'); ?>"
		,url_flag: "<?php echo site_urlg('circle/flag'); ?>"
		,url_sortid: "<?php echo site_urlg('circle/sortid'); ?>"
		,url_sort_change: "<?php echo site_urlg('circle/sort_change'); ?>"
		,url_copy: "<?php echo site_urlg('circle/copypro'); ?>"
	};
	ui.fancybox_img();
	ui.btn_delete(circle.url_del);		// 删除
	ui.btn_audit(circle.url_audit);	// 审核
	ui.btn_flag(circle.url_flag);		// 推荐
	// ui.sortable(circle.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(circle.url_sort_change); // input 排序
	ui.btn_copy(circle.url_copy);    // 热门审核
});
</script>
