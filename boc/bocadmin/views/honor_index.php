<div class="btn-group">
	<a href="<?php echo site_urlc('honor/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
require(['adminer/js/ui'],function(ui){
	var honor = {
		url_del: "<?php echo site_urlc('honor/delete'); ?>"
		,url_audit: "<?php echo site_urlc('honor/audit'); ?>"
		,url_flag: "<?php echo site_urlc('honor/flag'); ?>"
		,url_sortid: "<?php echo site_urlc('honor/sortid'); ?>"
		,url_sort_change: "<?php echo site_urlc('honor/sort_change'); ?>"
		,url_copy: "<?php echo site_urlc('honor/copypro'); ?>"
	};
	ui.fancybox_img();
	ui.btn_delete(honor.url_del);		// 删除
	ui.btn_audit(honor.url_audit);	// 审核
	ui.btn_flag(honor.url_flag);		// 推荐
	// ui.sortable(honor.url_sortid);	// 排序  拖动排序和序号排序在firefox中有bug
	ui.sort_change(honor.url_sort_change); // input 排序
	ui.btn_copy(honor.url_copy);    // 热门审核
});
</script>
