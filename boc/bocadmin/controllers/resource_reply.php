...
class Resource_Reply extends CRUD_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('demand_comment_model','mdemand_comment');
	...
	protected function _vdata(&$vdata)
  {
		...
		$demand_comment_info = $this->mdemand_comment->get_one($this->input->get('dcid'));

		$vdata['ctypes'] = $ctypes;
		$vdata['did'] = $demand_comment_info['did'];
	...
	protected function _index_where(){
		$where = array();
		$where['dcid'] = $this->input->get('dcid');
		return $where;
	}
