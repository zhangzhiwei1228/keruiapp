...
class Demand extends CRUD_Controller
{
	...
	protected function _vdata(&$vdata)
  {
	...
	protected function _index_where(){
		$where = array();
		if ($ctype = $this->input->get('ctype')) {
			$where['ctype'] = $ctype;
		}
		if ($province = $this->input->get('province_id')) {
			$where['province'] = $province;
		}
		if ($city = $this->input->get('city_id')) {
			$where['city'] = $city;
		}
		if ($title = $this->input->get('title')) {
			$where['like title'] = array('title', $title);
		}
		if ($content = $this->input->get('content')) {
			$where['like content'] = array('content', $content);
		}
		if ($tags = $this->input->get('tags')) {
			$where['like tags'] = array('tags', $tags);
		}
		return $where;
	}
	...
	protected function _edit_data(){
		...
		$form['ctype'] = $this->input->post('ctype');
		$form['tags'] = $this->input->post('tags');
		$form['content'] = $this->input->post('content');
		$form['photo'] = $this->input->post('photo');
		$form['thumb'] = $this->input->post('thumb');
		return $form;
	}
